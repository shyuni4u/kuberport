...
| VM | GCP `e2-medium` (2 vCPU / 4GB RAM / 30GB pd-balanced) | x86_64. 리전: `asia-northeast3` (서울) — 한국권 동료 레이턴시 ~30ms. 90일 크레딧 ($300) 으로 e2-medium 은 월 $25~35 수준이라 리전 선택 여유 있음. |
| OS | Ubuntu 24.04 LTS | |
...
| Postgres | in-cluster StatefulSet (pd-balanced PV) | 파일럿 데이터 유실 허용 (별도 백업 없음) — PV 자체는 영속이나 VM 터미네이트 / 프로젝트 삭제 시 사라짐 |
...
이 ADR 은 status=Proposed 이므로, Accepted 로 승격되기 전까지는 아래 문서를 수정하지 않는다 (프로젝트 문서가 "제안 중" 결정으로 오염되는 것을 막기 위함). **승격 시 별도 PR 로 일괄 반영한다**:
