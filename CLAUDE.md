...
**Plan 0~9 머지 완료.** 다음 단계: **dogfooding → 사용자 매뉴얼 → doc 업데이트 → Plan 10 (GCP Phase 1 부트스트랩)**. 진입 전 마스터 체크리스트는 [docs/release-prep.md](docs/release-prep.md) 참조. Plan 11 (e2e 확장) 은 Plan 10 운영 중 발견되는 버그 픽스와 함께 진행. Stage 2 reconciler 는 Plan 12 (deferred) — 다중 클러스터 / 페일오버 필요할 때 착수.
...
| 9 | [plan9-helm-chart](docs/superpowers/plans/2026-04-29-plan9-helm-chart.md) | ✅ merged (PR #38, #39) | **Helm chart MVP — Plan 10 의 하드 블로커 해소.** `deploy/helm/kuberport/` 단일 chart 에 backend / frontend / 옵셔널 in-cluster Postgres / Ingress / cert-manager Certificate / atlas schema apply (initContainer) 구성. 클라우드 중립 (Ingress class · StorageClass · 도메인만 values 분기). CI: `helm lint` + golden snapshot + kind smoke. 구현 중 hook → initContainer 로 전환 (chicken-and-egg). |
