...
**Plan 0~6 머지 완료**. 다음 단계로 Plan 7 (비주얼 리프레시 + i18n ko/en, Figma 레퍼런스 `nDP3cHNKf5Cjo6F2HU1EVv` 참조) 초안 작성 — 구현은 별도 진행. 그 후 방향(e2e 확장 / Helm chart 상세 / CRD 등) 은 미정.
...
| 7 | [plan7-visual-refresh](docs/superpowers/plans/2026-04-22-plan7-visual-refresh.md) | 📝 초안 | **비주얼 리프레시 + i18n(ko/en) — 기능·라우트·백엔드 변경 없음.** Figma 레퍼런스(`nDP3cHNKf5Cjo6F2HU1EVv`) 기반 디자인 토큰 재정의 + `AppShell`(좌측 사이드바 + 얇은 탑바) 도입 + `CatalogCard`/`ReleaseTable`/`RoleBadge`/`StatusChip`/`MetricCards` 비주얼 리프레시 + `next-intl` 로 사용자향 문자열 외부화 + 로케일 토글 UI. PR 이 과도히 커지면 7a(visual) / 7b(i18n) 로 분리. |
...
- 다음은 **원문(영어) 유지**: Conventional Commit 형식의 commit 메시지(`feat(scope): ...`), 코드 블록·로그 발췌·CLI 명령, 기술 식별자(함수명·파일 경로·환경변수).
- 본문 구조는 자유 — `## 요약` / `## 테스트 계획` 같은 한국어 헤더 권장. 체크박스(`- [ ]`)는 그대로.
